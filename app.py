...
</style></head><body><h1>Meetrapport generator</h1><!-- stable2-filecheck-UNIEK-2040 -->
...
function poll(id){fetch('/meetrapport-job-status/'+id).then(r=>r.json()).then(d=>{const p=d.percent||0;bar.style.width=p+'%';line.innerText=(d.step||'Bezig')+' - '+p+'%';detail.innerText=d.detail||'';if(d.status==='done'||d.status==='control_failed'){window.location.href='/meetrapport-job-result/'+id}else if(d.status==='error'){line.innerText='Fout - 100%';detail.innerText=d.detail||'Er ging iets mis.';btn.disabled=false;btn.innerText='Opnieuw proberen'}else{setTimeout(()=>poll(id),800)}}).catch(()=>setTimeout(()=>poll(id),1500))}
...
RESULT_HTML = """
<!doctype html><html><head><title>Meetrapport gemaakt</title><style>
body{font-family:Arial,sans-serif;max-width:820px;margin:40px auto}.btn{display:inline-block;padding:12px 18px;background:#1a73e8;color:white;text-decoration:none;border-radius:6px;font-weight:700;margin-right:8px}.okbox{padding:12px 14px;background:#eef8f0;border:1px solid #b8dfc0;margin:18px 0}.warnbox{padding:12px 14px;background:#fff6e5;border:1px solid #ffd58a;margin:18px 0}pre{white-space:pre-wrap;background:#f7f7f7;padding:12px;border-radius:6px}.check-table{border-collapse:collapse;width:100%;margin:18px 0}.check-table th{background:#f3f3f3;text-align:left}.check-table th,.check-table td{border-bottom:1px solid #ddd;padding:9px 8px}.yes{color:#0a7d2c;font-weight:700}.no{color:#d62828;font-weight:700}.warn{color:#c47a00;font-weight:700}.muted{color:#666;font-size:13px}.final-ok{font-weight:700;color:#0a7d2c}.final-warn{font-weight:700;color:#c47a00}.final-bad{font-weight:700;color:#d62828}
</style></head><body><h1>Meetrapport gemaakt</h1>{{message|safe}}{{check_html|safe}}<a class="btn" href="/download/{{pdf_id}}">Download meetrapport</a><a class="btn" href="/download/{{zip_id}}">Download zip</a><h3>Samenvatting</h3><pre>{{summary}}</pre></body></html>
"""

CONTROL_RESULT_HTML = """
<!doctype html><html><head><title>Controle bestanden</title><style>
body{font-family:Arial,sans-serif;max-width:820px;margin:40px auto}.badbox{padding:12px 14px;background:#fdecec;border:1px solid #f3b3b3;margin:18px 0}.check-table{border-collapse:collapse;width:100%;margin:18px 0}.check-table th{background:#f3f3f3;text-align:left}.check-table th,.check-table td{border-bottom:1px solid #ddd;padding:9px 8px}.yes{color:#0a7d2c;font-weight:700}.no{color:#d62828;font-weight:700}.warn{color:#c47a00;font-weight:700}.muted{color:#666;font-size:13px}.final-bad{font-weight:700;color:#d62828}.btn{display:inline-block;padding:10px 14px;background:#1a73e8;color:white;text-decoration:none;border-radius:6px;font-weight:700}
</style></head><body><h1>Controle bestanden</h1><div class="badbox">❌ Er is geen meetrapport gemaakt, omdat verplichte bestanden ontbreken.</div>{{check_html|safe}}<a class="btn" href="javascript:history.back()">Opnieuw uploaden</a></body></html>
"""
...
def job_done(job_id, pdf_id, zip_id, message, summary, check_html=""):
    with JOBS_LOCK:
        if job_id in JOBS: JOBS[job_id].update({"status":"done","step":"Klaar","percent":100,"detail":"Meetrapport en zip zijn klaar.","pdf_id":pdf_id,"zip_id":zip_id,"message":message,"summary":summary,"check_html":check_html})


def job_control_failed(job_id, check_html):
    with JOBS_LOCK:
        if job_id in JOBS: JOBS[job_id].update({"status":"control_failed","step":"Controle bestanden","percent":100,"detail":"Verplichte bestanden ontbreken. Er is geen rapport gemaakt.","check_html":check_html})
...
def has_go_token(filename):
    name = os.path.splitext(os.path.basename(filename).lower())[0]
    tokens = [t for t in re.split(r"[^a-z0-9]+", name) if t]
    return "go" in tokens


def is_meetstaat_name(filename):
    n = os.path.basename(filename).lower()
    return "meetstaat" in n or has_go_token(n)


def is_2d_name(filename):
    return "2d" in os.path.basename(filename).lower()


def is_3d_name(filename):
    return "3d" in os.path.basename(filename).lower()


def is_fml_name(filename):
    return os.path.basename(filename).lower().endswith(".fml")


def is_image_or_pdf(filename):
    return os.path.basename(filename).lower().endswith((".jpg", ".jpeg", ".png", ".webp", ".pdf"))
...
def classify_files(paths):
    ...
    for p in paths:
        ...
        if is_fml_name(n): continue
        if not is_image_or_pdf(n): continue
        if is_meetstaat_name(n): meetstaat.append(p)
        elif is_3d_name(n): p3d.append(p)
        elif is_2d_name(n): p2d.append(p)
        else: other.append(p)
    return meetstaat, p2d, p3d, other


def analyze_upload_files(paths):
    files = [os.path.basename(p) for p in paths]
    meetstaat = [f for f in files if is_meetstaat_name(f) and is_image_or_pdf(f)]
    p2d = [f for f in files if is_2d_name(f) and is_image_or_pdf(f)]
    p3d = [f for f in files if is_3d_name(f) and is_image_or_pdf(f)]
    fml = [f for f in files if is_fml_name(f)]
    rows = [
        {"label":"Meetstaat of GO in naam", "ok": bool(meetstaat), "required": True, "files": meetstaat},
        {"label":"2D in naam gevonden", "ok": bool(p2d), "required": True, "files": p2d},
        {"label":"3D in naam gevonden", "ok": bool(p3d), "required": True, "files": p3d},
        {"label":".fml bestand aanwezig", "ok": bool(fml), "required": False, "files": fml},
    ]
    required_ok = all(r["ok"] for r in rows if r["required"])
    warnings = [r for r in rows if (not r["required"] and not r["ok"])]
    return {"rows": rows, "required_ok": required_ok, "warnings": warnings}


def render_file_check_table(check):
    html_rows = []
    for r in check["rows"]:
        if r["ok"]:
            result = '<span class="yes">JA</span>'
        elif r["required"]:
            result = '<span class="no">NEE</span>'
        else:
            result = '<span class="warn">NEE</span> <span class="muted">(waarschuwing)</span>'
        names = "<br>".join(html.escape(x) for x in r["files"]) if r["files"] else '<span class="muted">Geen bestand gevonden</span>'
        html_rows.append(f"<tr><td>{html.escape(r['label'])}</td><td>{result}</td><td>{len(r['files'])}</td><td>{names}</td></tr>")
    if check["required_ok"] and check["warnings"]:
        final = '<p>Eindresultaat: <span class="final-warn">LET OP: .fml ontbreekt, rapport wordt wel gemaakt</span></p>'
    elif check["required_ok"]:
        final = '<p>Eindresultaat: <span class="final-ok">OK - bestanden compleet</span></p>'
    else:
        final = '<p>Eindresultaat: <span class="final-bad">GEEN RAPPORT - verplichte bestanden ontbreken</span></p>'
    return '<h3>Bestandscontrole</h3><table class="check-table"><thead><tr><th>Controle</th><th>Resultaat</th><th>Aantal</th><th>Bestanden</th></tr></thead><tbody>' + "".join(html_rows) + '</tbody></table>' + final
...
def make_outputs(data, upload_paths, work_dir):
    ...
    if not meetstaat: raise RuntimeError("Geen meetstaat of GO-bestand gevonden. Zorg dat 'Meetstaat' of 'GO' in de bestandsnaam staat.")
    if not p2d: raise RuntimeError("Geen 2D-bestand gevonden. Zorg dat '2D' in de bestandsnaam staat.")
    if not p3d: raise RuntimeError("Geen 3D-bestand gevonden. Zorg dat '3D' in de bestandsnaam staat.")
...
def process_job(job_id, record_id, paths, work_dir, form_data=None):
    try:
        ...
        set_job(job_id,"Bestanden controleren",14,"Bestandsnamen worden gecontroleerd op Meetstaat/GO, 2D, 3D en .fml...")
        check = analyze_upload_files(paths)
        check_html = render_file_check_table(check)
        if not check["required_ok"]:
            job_control_failed(job_id, check_html)
            return
        if record_id:
        ...
        if check["warnings"]:
            msg += " ⚠️ Let op: .fml-bestand ontbreekt."
        if record_id:
            ...
            if check["warnings"]:
                msg += "<br>⚠️ Let op: .fml-bestand ontbreekt."
        box = f'<div class="okbox">{msg}</div>' if msg.startswith("✅") else f'<div class="warnbox">{msg}</div>'
        job_done(job_id,pdf_id,zip_id,box,summary(data,counts),check_html)
...
def health(): return {"status":"ok","version":"stable2-filecheck-UNIEK-2040"}
...
def job_result(job_id):
    ...
    if job.get("status")=="control_failed": return render_template_string(CONTROL_RESULT_HTML, check_html=job.get("check_html", ""))
    ...
    return render_template_string(RESULT_HTML,pdf_id=job["pdf_id"],zip_id=job["zip_id"],message=job["message"],summary=job["summary"],check_html=job.get("check_html", ""))

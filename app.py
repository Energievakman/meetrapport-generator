...
SOFTR_POSTCODE_GEMEENTE_FIELD = os.environ.get("SOFTR_POSTCODE_GEMEENTE_FIELD", "Postcode & gemeente")
...
button{padding:10px 16px;cursor:pointer}.dropzone{border:2px dashed #b8b8b8;border-radius:12px;padding:24px;background:#fafafa;margin:16px 0;max-width:680px}.dropzone.dragover{border-color:#1a73e8;background:#eef5ff}.row{margin:14px 0}label{font-weight:700;display:block;margin-bottom:6px}input[type=file]{display:block;margin-top:8px}input[type=text]{padding:8px;width:360px}.small{font-size:13px;color:#666}#progressBox{display:none;margin-top:22px;max-width:680px}.progress-wrap{height:18px;background:#eee;border-radius:999px;overflow:hidden}.progress-bar{height:18px;width:0%;background:#1a73e8;transition:width .25s}.progress-line{margin-top:10px;font-weight:700}.progress-detail{color:#666;margin-top:5px;font-size:14px}
</style></head><body><h1>Meetrapport generator</h1>
<p class="hint">Upload meetstaat + 2D/3D plattegronden. De .fml mag mee, maar wordt niet opgenomen in het rapport en ook niet in de zip.</p>
<form id="uploadForm" action="{{action_url}}" method="post" enctype="multipart/form-data">
<div class="row"><label>Adres vol / bestandsnaam</label><input type="text" name="adres_vol" placeholder="Bijv. Vondelstraat 185 Den Haag Nederland"></div>
<div class="row"><label>Adres</label><input type="text" name="adres" placeholder="Bijv. Vondelstraat 185"></div>
<div class="row"><label>Postcode & gemeente</label><input type="text" name="postcode_gemeente" placeholder="Bijv. 2513ET 's-Gravenhage"></div>
<div class="row"><label>Opdrachtgever</label><input type="text" name="opdrachtgever" placeholder="Bijv. Wooncompany"></div>
<div class="row"><label>Adviseur</label><input type="text" name="adviseur" placeholder="Bijv. Otto Boender"></div>
<div class="dropzone" id="dropzone"><b>Bestanden</b><div class="small">Meetstaat, 2D plattegronden, 3D plattegronden. Meerdere bestanden tegelijk selecteren.</div><input id="fileInput" type="file" name="files" multiple accept="image/*,application/pdf,.pdf,.fml"></div>
...
<script>
const dz=document.getElementById('dropzone'),fi=document.getElementById('fileInput'),form=document.getElementById('uploadForm'),btn=document.getElementById('submitBtn'),box=document.getElementById('progressBox'),bar=document.getElementById('progressBar'),line=document.getElementById('progressLine'),detail=document.getElementById('progressDetail');
dz.addEventListener('dragover',e=>{e.preventDefault();dz.classList.add('dragover')});dz.addEventListener('dragleave',()=>dz.classList.remove('dragover'));dz.addEventListener('drop',e=>{e.preventDefault();dz.classList.remove('dragover');fi.files=e.dataTransfer.files});
...
def create_meetrapport(data, meetstaat_imgs, p2d_imgs, p3d_imgs):
    ...
    draw_logo(c,42,h-40); c.setFont("Helvetica",10); c.setFillColor(TEXT); c.drawString(365,h-58,"Energielabels & Advies")
    c.setFillColor(colors.HexColor("#707a6d")); c.roundRect(510,h-62,58,24,5,fill=1,stroke=0); c.setFillColor(colors.white); c.setFont("Helvetica-Bold",8); c.drawCentredString(539,h-47,"NEN2580")
    ...
    for img in meetstaat_imgs:
        fit_image(c,img,28,120,w-56,h-190); draw_footer(c,footer); c.showPage()
    # 2d pages
    for img in p2d_imgs:
        fit_image(c,img,28,70,w-56,h-110); draw_footer(c,footer); c.showPage()
    # 3d: two per page when possible
    for i in range(0,len(p3d_imgs),2):
        fit_image(c,p3d_imgs[i],28,h/2+10,w-56,h/2-70)
        if i+1 < len(p3d_imgs): fit_image(c,p3d_imgs[i+1],28,60,w-56,h/2-70)
        draw_footer(c,footer); c.showPage()
...
def make_zip(zip_name, source_paths, pdf_bytes, pdf_name):
    ...
    with zipfile.ZipFile(b,"w",zipfile.ZIP_DEFLATED) as z:
        ...
        for p in source_paths:
            ...
            low=n.lower()
            if low.endswith(".fml") or "meetstaat" in low or low.startswith("meet"):
                continue
            arc=n; i=2
            while arc in seen:
...
def data_from_form():
    return {k: request.form.get(k, "").strip() for k in ["adres_vol","adres","postcode_gemeente","opdrachtgever","adviseur"]}

def data_from_record(record):
    f=record.get("fields", {})
    return {
        "adres_vol": first_value(f, SOFTR_ADRES_VOL_FIELD) or first_value(f, SOFTR_ADRES_FIELD),
        "adres": first_value(f, SOFTR_ADRES_FIELD),
        "postcode_gemeente": first_value(f, SOFTR_POSTCODE_GEMEENTE_FIELD),
        "opdrachtgever": first_value(f, SOFTR_OPDRACHTGEVER_FIELD),
        "adviseur": first_value(f, SOFTR_ADVISEUR_FIELD),
    }

def summary(data, counts):
    return "\n".join([f"adres_vol: {data.get('adres_vol','')}",f"adres: {data.get('adres','')}",f"postcode_gemeente: {data.get('postcode_gemeente','')}",f"opdrachtgever: {data.get('opdrachtgever','')}",f"adviseur: {data.get('adviseur','')}",f"meetstaat_bestanden: {counts.get('meetstaat',0)}",f"2d_plattegronden: {counts.get('2d',0)}",f"3d_plattegronden: {counts.get('3d',0)}"])
...
def index(): return render_template_string(HTML, action_url="/meetrapport-upload")
...
def upload_for_record(record_id):
    if request.method=="GET": return render_template_string(HTML, action_url=f"/meetrapport-upload-for-record/{record_id}")
    files=request.files.getlist("files")
    if not files: return jsonify({"error":"Geen bestanden ontvangen"}),400
    job_id=uuid.uuid4().hex; work=os.path.join(TEMP_DIR,"meetrapport_job_"+job_id); os.makedirs(work,exist_ok=True)
    paths=[]
